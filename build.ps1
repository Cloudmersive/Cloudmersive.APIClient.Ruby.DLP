...
& java -jar swagger-codegen-cli-2.4.14.jar generate -i .\dlp-api-swagger.json -l ruby -o client -c packageconfig.json
...
(Get-Content ./client/cloudmersive-dlp-api-client.gemspec).replace('`find *`.split("\n").uniq.sort.select { |f| !f.empty? }', "Dir['./**/*']") | Set-Content ./client/cloudmersive-dlp-api-client.gemspec
#& npm build ./client
